...
스킬별 미해결은 각 스펙 «구현자에게 남기는 미확정» 절의 «미해결 — 2026-09-24»에 있다. 여기는 플러그인 수준만 둔다. 해소하면 지운다.

- **LOW** — `scripts/sync.mjs`의 폴더 밖 경로 검사가 `../../`만 찾는다. `../` 한 단계 이탈(스킬 폴더에서 `skills/`로 나가는 경로)은 못 잡는다.
