...
| 발견 | 조치 |
| --- | --- |
| `ios/min-deployment-target`의 1차·2차가 **둘 다 Xcode/SDK 요구만 싣고 최소 배포 타깃 재료가 없다** | 실측 필드를 사실대로 고치고 **미해결로 남겼다** — URL 재선정은 스펙이 구현 재량으로 위임한 축이다(감사 원장 A-29 — git 태그 `audit-2026-08`의 `docs/audit-ledger.md`) |
