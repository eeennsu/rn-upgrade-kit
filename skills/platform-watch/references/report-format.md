...
- **보존 상한** = `references/constants.md`의 `report_retention_n`. **초과분을 지우지 않는다 — 보고만 한다.** 이 스킬은 셸을 호출하지 않고(`SKILL.md` 서두) `Write`는 덮어쓰기뿐이라 **파일을 지울 수단 자체가 없다.** 없는 수단의 실행 결과를 리포트에 적으면 그게 환각이다 — 이 스킬의 1번 제약 위반이다.
...
🟠 · ios/min-xcode · SDK iOS 27 (예고)   [변경 Xcode 26 → SDK 27]
   ...
   근거: 예고 https://developer.apple.com/app-store/submitting/ · 발효 https://developer.apple.com/news/upcoming-requirements/
...
>
> 예시의 D-day는 헤더 날짜(2026-09-10) 기준이다: 2026-08-31 → `D+10`, 2026-09-09 → `D+1`, 2027-02-01 → `D-144`. `ios/min-xcode`의 `Starting April 2027`은 예고 URL(제출 안내 페이지)에서 온 것이다 — 1차에는 아직 없다(2026-09-24 실측 · `references/watch-targets.md`). 발효 요구(Xcode 26)를 맞췄으므로 예고가 마감이 됐고, 초일 2027-04-01까지 203일이라 임계일 밖 🟠다.

- 각 항목에 **슬러그를 함께 출력한다.**
- **헤더 `프로젝트:` 줄은 항상 싣는다** — `references/expo.md` §1의 형식이다. Expo가 아니면 `프로젝트: RN (Expo 아님)`이다.
- **Expo 프로젝트의 `현재:` 줄은 출처를 꼬리로 단다** — `35 (app.json expo-build-properties)` · `36 (Expo SDK 57 기본값)` · `16.4 (ios/Podfile.properties.json)` · `≥ 26.4 (Expo SDK 57 최소 Xcode)`. SDK 기본값이면 문서 표 링크를 **꼬리 밖** 근거 자리에 단다(`현재: 36 (Expo SDK 57 기본값) · 근거: <표 링크>`) — 핸드오프 `current`에 URL이 섞이지 않게 하려는 것이다. 위 숫자는 값이 아니라 예시다.
...
- **예고 URL이 도달 실패면 그 항목 줄에 `예고 확인 못 함 (예고 URL 도달 실패)`를 병기한다** — `이중화 없음`과 같은 소스 상태 꼬리표다. 판정은 1차로 이미 섰으므로 블록·등급은 바뀌지 않는다. **예고가 없어서 못 가져온 것은 표기하지 않는다** — 그건 정상 상태다(SKILL.md §2 «예고 URL»). 둘을 섞으면 예고 URL이 죽은 것과 예고가 아직 없는 것이 같아 보인다.
- **근거 링크는 값을 실은 페이지다.** 예고가 마감이 된 항목은 예고 URL과 발효 요구의 1차를 둘 다 단다(`근거: 예고 <URL> · 발효 <URL>`).
...
경로는 `references/constants.md`의 `handoff_path`. **소유자는 이 스킬, 독자는 `currency`. 단방향이다** — `currency`의 산출물을 읽지 않는다.
...
> 위 예시의 `1`은 값이 아니라 예시다 — 실제 값은 `references/constants.md`의 `handoff_schema_version`에서 온다.
>
...
- **`current`는 리포트 `현재:` 줄의 값과 출처 꼬리까지다** — `(Expo SDK <n> 기본값)` 같은 꼬리는 싣고, `· 근거: <링크>`는 싣지 않는다. `currency`는 이 값을 스냅샷 헤더에 옮기기만 한다(계약 5).
...
- **`source`는 `requires`·`deadline`을 실은 페이지다.** 예고가 마감이 됐으면 예고 URL이다 — `currency`가 번역 근거를 따라갈 곳이 그 페이지다. 예고 URL이 도달 실패면 `source` 뒤에 `(예고 확인 못 함)`을 붙인다.
