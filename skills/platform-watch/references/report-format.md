...
- **산출물 루트 후보가 둘 이상이거나 없으면 리포트 헤더에 `산출물 루트: <경로>`를 싣는다.** 기준은 `../../../shared/constants.md`의 «산출물 루트»다 — 모노레포에서 이 스킬과 `currency`가 다른 자리를 보면 **핸드오프가 조용히 끊기고** `currency`는 그걸 «파일 없음»(=한 번도 안 돌렸다)으로 읽는다. 어디에 썼는지가 리포트에 없으면 원인을 추적할 자리가 없다.
...
- **병합 보존 대상은 enum에 있는 슬러그만이다** — 아래 «enum에서 제거된 슬러그» 참조.
- **`entries.<슬러그>.withdrawal_suspect_count`는 요구 폐지 의심의 연속 관측 수다.** SKILL.md §6의 degrade 5-a가 임계(`../../../shared/constants.md`의 `withdrawal_suspect_min_count`)를 그 값으로 판정한다. 이번 실행에서 그 항목의 재료가 다시 안 보이면 +1, **재료가 돌아오면 필드를 지운다** — 연속이 끊겼기 때문이다. 상태 파일이 없거나 깨졌으면 셀 수 없으므로 **제안 절만 생략**하고 `확인 못 함 (요구 폐지 의심 …)` 표기 자체는 그대로 낸다(degrade 1과 같은 모양).
...
`references/watch-targets.md`에서 제거·통합된 슬러그의 **상태 파일 엔트리는 다음 실행(스코프 무관)에서 삭제한다.** 리포트 말미에 한 줄로 알린다 — 예: `enum 정리: ios/min-deployment-target 엔트리 삭제 — ios/min-xcode로 통합`.

- 남겨두면 병합 규칙이 폐기된 항목을 **영구 보존**하고, 그 엔트리가 언젠가 `[신규]`·델타 오탐의 재료가 된다.
- **병합 규칙이 "삭제 금지"를 말하는 대상은 미조회이지 폐기가 아니다.** 둘은 반대 방향이다 — 미조회는 아직 안 본 것이고 폐기는 더 볼 필요가 없어진 것이다.
- 핸드오프 파일 쪽 처리는 §3의 같은 이름 절에 있다. **두 파일이 같은 슬러그를 서로 다른 시점에 지우면 `currency`가 한쪽에만 남은 항목을 본다.**
...
- **`status`의 값 집합도 고정이다: `충족` · `미충족` · `판정 불가`.** 다른 문구를 쓰지 마라 — `currency`가 **문자열 `미충족`에 직접 분기한다**(계약 6의 예외). `충족 아님`처럼 표기만 바꿔도 그 예외가 조용히 죽고 **이미 지난 마감이 `🟠 여유`로 나간다.** `urgency`만 열거하고 `status`를 안 하면, 같은 분기에 함께 쓰이는 두 필드 중 하나만 계약이 되는 비대칭이 생긴다.
  - `충족` = 프로젝트 현재값이 요구를 만족 / `미충족` = 만족하지 않음 / `판정 불가` = 현재값 또는 요구를 확정하지 못함
  - **예외는 하나 — 한 번도 조회된 적 없는 항목이다.** 그 항목은 **모든 필드**에 고정 표기 `미조회 (한 번도 조회되지 않음)`가 들어가므로 `status`도 3값 밖이다. `urgency`에 같은 예외가 있는 것과 같고(아래), 소비자도 그 문자열을 3값 어디에도 매칭하지 않고 `(핸드오프 미조회 항목)`으로 받는다. **조회한 적 없는 것에 판정값을 주지 않는다** — 그게 이 고정 표기가 존재하는 이유다.
...
«enum 전체를 싣는다»의 enum은 **현재의** `references/watch-targets.md`다. 거기서 제거·통합된 슬러그의 **핸드오프 섹션은 다음 실행에서 삭제하고**, 통합이면 후속 슬러그를 리포트 말미에 알린다.

- 남겨두면 `currency`가 **폐기된 요구를 살아 있는 하한으로 읽는다.**
- **병합 규칙이 "삭제 금지"를 말하는 대상은 미조회이지 폐기가 아니다.** 미조회 항목은 고정 표기로 자리를 지키고, 폐기 항목은 자리를 잃는다.
- 상태 파일 쪽 처리는 §2의 같은 이름 절에 있다. **한 실행 안에서 둘을 같이 지운다** — 한쪽만 지우면 다음 실행이 남은 쪽을 근거로 엔트리를 되살린다.
...
6. `urgency`는 정책 하한 항목의 🔴/🟠 분기의 **유일한 공급원**이다. **예외 하나 — 기한 경과**: `deadline`이 `currency` 실행 시점보다 과거이고 `status`가 `미충족`이면, `urgency` 값과 무관하게 🔴 + `기한 경과`를 병기한다.

> **계약 6의 예외가 생산자 쪽에도 적혀 있어야 하는 이유**: 그 예외는 `deadline`과 `status` **두 필드를 이 파일의 포맷 그대로** 읽는다 — 저정밀 날짜의 정렬 초일 표기(`2027-04 (저정밀 — 정렬 초일 2027-04-01)`)까지 포함해서다. 이 파일만 읽는 유지보수자가 `deadline` 포맷이나 `status` 문구를 바꾸면 예외가 조용히 죽고, 낙관 편향 금지를 위해 만든 장치가 사라진 줄도 모른다. **계약은 양쪽에 적혀야 성립한다.**
