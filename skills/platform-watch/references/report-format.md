...
- **`entries.<슬러그>.withdrawal_suspect_count`는 요구 폐지 의심의 연속 관측 수다.** SKILL.md §6의 degrade 5-a가 임계(2회)를 그 값으로 판정한다. 이번 실행에서 그 항목의 재료가 다시 안 보이면 +1, **재료가 돌아오면 필드를 지운다** — 연속이 끊겼기 때문이다. 상태 파일이 없거나 깨졌으면 셀 수 없으므로 **제안 절만 생략**하고 `확인 못 함 (요구 폐지 의심 …)` 표기 자체는 그대로 낸다(degrade 1과 같은 모양).
