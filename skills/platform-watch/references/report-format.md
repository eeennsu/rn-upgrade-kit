...
- **Expo 프로젝트의 `현재:` 줄은 출처를 꼬리로 단다** — `35 (app.json expo-build-properties)` · `36 (Expo SDK 57 기본값)` · `16.4 (ios/Podfile.properties.json)`. SDK 기본값이면 그 줄에 문서 표 링크를 근거로 단다(`../SKILL.md` §4 «Expo 프로젝트»). 위 숫자는 값이 아니라 예시다.
...
- **`current`는 리포트 `현재:` 줄과 같은 문자열이다** — Expo 프로젝트의 출처 꼬리(`(Expo SDK <n> 기본값)` 등)까지 그대로 싣는다. `currency`는 이 값을 헤더에 옮기기만 한다(계약 5).
