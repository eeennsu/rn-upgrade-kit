...
- **부팅과 로그 스캔은 한 단계로 묶여 `step_timeout_boot_seconds` 상한 안에서 돈다**(`references/constants.md`). 초과하면 `실패 (타임아웃 step_timeout_boot_seconds)`다 — 위 «셋 다 못 얻음»의 `실패 (첫 프레임 신호 관측 실패)`와 **사유를 합치지 마라.** 앞은 기다리다 끊은 것이고 뒤는 끝까지 봤는데 신호가 없던 것이라, 독자가 할 일이 다르다(상한 조정 대 실제 회귀 조사).
