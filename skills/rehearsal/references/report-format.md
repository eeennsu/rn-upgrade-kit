...
위 표의 버전은 값이 아니라 예시다. 순서의 이유: Expo 프로젝트에서는 SDK가 RN까지 고정하므로 `expo`가 업그레이드의 주어다. 그 밖에서는 RN이 lockstep 반경이 가장 넓고 리허설의 사실상 주어다. **`<target>`은 식별자일 뿐 내용이 아니다** — 전체 세트는 리포트 헤더와 채택 커밋 메시지에 전부 실린다.
...
`artifacts/`는 최근 N개만 보존한다. N = `references/constants.md`의 `artifact_retention_n`. **정리한 개수를 리포트 말미에 한 줄로 보고한다.**
...
> 검사 범위 밖이다.
(expo-dev-client가 직접 선언된 Expo 프로젝트에서 T2 통과가 있으면 이 줄을 더한다)
T2 통과 해석 한계: expo-dev-client가 있어 debug 빌드는 개발 런처를 띄울 수 있다 — 관측한 화면이 앱이 아니라 런처일 수 있다.
...
| `베이스라인: 미측정` 또는 `베이스라인: 측정됨 (<사유>)` 또는 `베이스라인: 미측정 (되돌림 실패: <사유>)` | **항상** |
| `커밋 외 파일: worktree 미포함 N개 — <목록>` 또는 `커밋 외 파일: 없음` | **항상** (`../SKILL.md` §0 «커밋 외 파일 · 격리 누수») |
...
| `미등록 패치: <파일>` | `patches/`에 있으나 PM에 등록되지 않은 파일 발견 시 |

- **`base_sha` 표기 자릿수는 자리마다 다르다.** 리포트 **본문**에 값으로 싣는 자리(`검증 기준` 줄 · 채택 절 · 채택 커밋 메시지 · base 신선도 경고문 · 재현 블록의 git 인자)는 **전체 SHA**다. **브랜치명과 worktree 경로만 7자리**이고 그건 `references/constants.md`의 `worktree_path_template`이 `<base_sha7>`로 못박은 값이다. 섞으면 사후 감사에서 *"이 브랜치가 검증한 커밋"*을 짧은 해시로만 알게 되는데, 리포트는 몇 달 뒤에 읽히고 **짧은 해시는 레포가 커지면 충돌한다.**
...
- `인자 검증 1` · `인자 검증 3` · `인자 검증 4` · `artifacts 쓰기 실패` 줄은 **degrade의 흔적**이다(`../SKILL.md` §8). 줄이 없으면 degrade가 없었다는 뜻이어야 하므로 **degrade가 있었는데 안 적는 경우가 있어서는 안 된다** — 조용히 열화된 리허설은 정상 실행과 구분되지 않는다.
...
8. **worktree 경로는 `references/constants.md`의 `worktree_path_template`에서 온다.** 블록에 직접 적힌 경로를 정본으로 삼지 마라 — 재현 블록·수동 정리 커맨드·충돌 판정이 같은 값을 봐야 한다.
...
- **`프로젝트:` 줄을 싣는다.** Expo CNG면 네이티브 디렉토리가 커밋에 없는 이유가 이 줄로 읽힌다(`../SKILL.md` §6 «Expo CNG»).
