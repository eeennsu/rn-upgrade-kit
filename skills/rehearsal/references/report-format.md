...
T2 통과 = 첫 프레임 렌더 + 생존 + 로그 클린. 어느 화면까지 갔는지는 관측하지 않는다 — 인증 이후는 미검증(비범위).
...
- **T2 상세에는 판정선 고정 문구를 싣는다** — `T2 통과 = 첫 프레임 렌더 + 생존 + 로그 클린. 어느 화면까지 갔는지는 관측하지 않는다 — 인증 이후는 미검증(비범위).`(`../SKILL.md` §3). 통과·실패 어느 쪽이든 싣는다: **무엇을 보지 않았는지가 통과일 때 더 중요하다.**
...
| `산출물 루트: <경로>` + 나머지 후보 목록 | RN 프로젝트 루트 후보가 **둘 이상인** 경우 (후보가 없으면 실행 거부라 리포트가 없다) |
