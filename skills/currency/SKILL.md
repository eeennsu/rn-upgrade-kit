...
- **19를 «대상 0개 → 종료»로 접지 마라.** §1은 `dependencies`가 비었을 때 "RN 앱이 아니다"라는 뜻의 범위 안내를 내는데, 파싱 실패는 **RN 앱인지 아닌지조차 모르는 상태**다. 같은 문구를 쓰면 사용자는 깨진 `package.json`을 고치는 대신 스킬을 잘못 불렀다고 생각한다. **모른다는 것과 없다는 것은 다르다.**
