...
- **보존 상한** = `references/constants.md`의 `report_retention_n`. 초과분을 **지우지 않고 보고한다** — 말미에 한 줄과 **초과 파일 경로 목록**을 낸다.
...
- **표기는 `platform-watch`와 글자까지 같아야 한다.** 두 advisory 스킬은 보존 규칙이 같다고 `references/constants.md`가 못박고 있다 — 문구가 갈리면 사용자가 두 리포트를 다른 정책으로 읽는다.
...
> 위 예시의 `12`·`1`은 값이 아니라 예시다 — 실제 보존 상한은 `references/constants.md`의 `report_retention_n`에서 온다.
