...
**위 표는 태그 URL 조립용이지 lockstep 정의가 아니다.** 같은 repo에 있다는 사실과 짝으로만 올려야 한다는 사실은 다르다 — **lockstep 세트의 정본은 `references/lockstep-sets.md` 하나뿐**이고 게이트 6은 그 파일만 읽는다. 이 표를 세트 목록으로 겸용하면 모노레포 항목이 전부 lockstep으로 오해되고, 표에 없는 짝은 짝이 아닌 것으로 오해된다.
