...
사본·정본 관계와 도달 방법은 같은 폴더 `constants.md`의 «사본과 정본»·«이 파일에 도달하는 법»과 같다 — 이 파일도 `currency`·`rehearsal`의 `references/lockstep-sets.md`에 사본으로 들어 있고 정본은 repo의 `shared/lockstep-sets.md`다. **확정 세트 추가는 정본에서 하라** — 사본 하나만 고치면 `currency`와 `rehearsal`이 다른 목록을 본다. **못 읽으면 지어내지 마라** — 아래 «도달 실패» 참조.
...
## Expo 프로젝트 — SDK 정합은 별개 규칙이다

Expo 프로젝트에는 위 세트 위에 **SDK 정합 규칙**(같은 폴더 `expo.md` §5)이 하나 더 걸린다. **둘은 별개이고 둘 다 통과해야 한다.**

- **SDK 호환 목록은 lockstep 세트가 아니다.** 목록 전체를 짝으로 함께 올리라는 뜻이 아니다. 목표 SDK가 정한 범위를 만족하라는 뜻이다. 이미 범위 안에 있는 패키지는 올리지 않아도 된다 — **단 이 파일의 세트 구성원이면 세트 규칙이 우선한다.** 짝이 세트에 들어가면 버전이 그대로여도 현재 버전으로 함께 싣는다(`expo.md` §5).
- **SDK 목록에 없는 세트 구성원도 lockstep 규칙은 그대로 받는다.** 예: `@react-native/*`가 SDK 목록에 없어도, 직접 선언돼 있으면 RN과 같은 번호로 동반한다.
...
| 2 | 같은 GitHub 모노레포에서 **한 태그로 함께 배포** | `currency`의 `references/sources.md` repo/태그 표 |
