...
세 스킬이 **같은 값을 봐야 하는** 상수의 정본. 스킬 본문·다른 참조 파일에 숫자를 복제하지 마라 — 한쪽만 바뀌는 드리프트가 이 파일이 존재하는 이유다.

## 사본과 정본

이 파일은 **각 스킬 폴더에 사본으로 들어 있다** — `<스킬>/references/constants.md`. 스킬 폴더 하나만 복사해 가는 설치 경로(`npx skills` · 스킬 단위 업로드)에서도 스킬이 자기 폴더 안에서 상수를 읽게 하려는 것이다.

- **정본은 rn-upgrade-kit repo의 `shared/constants.md` 하나다.** 사본은 `node scripts/sync.mjs`가 만든다.
- **사본을 직접 고치지 마라.** 사본이 스킬마다 있으므로 한 곳만 고치면 스킬끼리 다른 값을 본다 — 이 파일이 막으려던 드리프트를 사본이 만든다. repo에서는 `node scripts/sync.mjs --check`(CI)가 어긋난 사본을 잡는다. **설치본에는 이 검사가 없다** — 설치본에서 값을 바꾸려면 모든 스킬의 사본을 같이 고쳐라.

## 이 파일에 도달하는 법

스킬은 **스킬 루트 기준** 경로로 Read한다 — `references/constants.md`. `SKILL.md`에서도 `references/*.md`에서도 같은 경로다.

- **해석 기준이 스킬 루트라는 보장은 없다.** 호출 시점의 작업 디렉토리는 사용자 RN 프로젝트다. 위 경로는 **스킬 로드 시 주어진 자기 `SKILL.md`의 폴더(스킬 루트)를 기준으로 절대경로로 풀어 Read하라**는 뜻이다 — 작업 디렉토리 기준으로 풀면 실패한다.
- 그 Read가 실패하면 **`Glob`으로 `**/<자기 스킬 이름>/references/constants.md`를 찾아 그 경로로 다시 Read한다.** 단 **`Glob`은 작업 디렉토리 기준이라 스킬이 사용자 프로젝트 트리 안에 있을 때(로컬 개발 · 프로젝트 단위 설치)만 잡는다** — 전역 설치본(`~/.claude/**` 계열)은 이 폴백이 못 찾는다. 폴백이 있으니 degrade가 드물 거라고 가정하지 마라 — 실효 방어선은 첫 Read다.
...
`grade_threshold_days`만 항목별로 덮어쓸 수 있다 — `platform-watch`의 `references/watch-targets.md` 항목 필드. 나머지는 전역이다.
