...
- **둘 다 실패하면 숫자를 지어내지 마라.** 세 스킬 전부 `상수 도달 실패` degrade 경로를 가지지만 **처리가 둘로 갈린다.** 두 advisory(`platform-watch`·`currency`)는 상수에 기대는 판정만 `확인 못 함`으로 두고, 그 사실을 리포트 헤더에 적고, 나머지 리포트는 정상 산출한다. **`rehearsal`은 실행 거부다** — 거기서 상수는 판정 임계값이 아니라 **실행 파라미터**라(타임아웃 없이는 단계가 무한정 멈추고, `worktree_path_template` 없이는 격리 공간을 어디에 만들지도 모른다) 실행 자체가 성립하지 않고, `확인 못 함`은 그 스킬의 어휘도 아니다. 정본은 그 스킬의 §8 degrade 1이다. 이 파일의 존재 이유가 *"한쪽만 바뀌는 드리프트 방지"*인데 못 읽었을 때 추정값을 쓰면 드리프트를 막는 대신 **만들어낸다.**
